...
var hyperscriptVnode = function () {
  ...
  if (attrs1 == null) {
    ...
    typeof attrs1 !== 'object' ||
    attrs1.tag != null ||
    Array.isArray(attrs1)
  ) {
    attrs1 = {}
    start = this
  }
...
var _13 = function ($window) {
  ...
  function createHTML(parent, vnode3, ns, nextSibling) {
    ...
    if (ns === 'http://www.w3.org/2000/svg') {
      temp.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg">' + vnode3.children + '</svg>'
  ...
  function initComponent(vnode3, hooks) {
    ...
    if (typeof vnode3.tag.view === 'function') {
      ...
      sentinel.$$reentrantLock$$ = true
      vnode3.state =
        vnode3.tag.prototype != null &&
        typeof vnode3.tag.prototype.view === 'function'
  ...
  function removeNode(parent, vnode3) {
    ...
    if (
      typeof vnode3.tag !== 'string' &&
      typeof vnode3.state.onbeforeremove === 'function'
    ) {
  ...
  function onremove(vnode3) {
    if (
      typeof vnode3.tag !== 'string' &&
      typeof vnode3.state.onremove === 'function'
    )
  ...
  function setAttrs(vnode3, attrs2, ns) {
    ...
    if (vnode3.tag === 'input' && attrs2.type != null)
      vnode3.dom.setAttribute('type', attrs2.type)
    var isFileInput =
      attrs2 != null && vnode3.tag === 'input' && attrs2.type === 'file'
  ...
  function setAttr(vnode3, key, old, value, ns, isFileInput) {
    if (
      ...
      (old === value &&
        !isFormAttribute(vnode3, key) &&
        typeof value !== 'object') ||
      (key === 'type' && vnode3.tag === 'input')
    )
      return
    if (key[0] === 'o' && key[1] === 'n') return updateEvent(vnode3, key, value)
    if (key.slice(0, 6) === 'xlink:')
      vnode3.dom.setAttributeNS(
      ...
      if (key === 'value') {
        ...
        if (
          ...
          (isFileInput || vnode3.dom === activeElement())
        )
          return
  ...
  function shouldNotUpdate(vnode3, old) {
    do {
      if (
        vnode3.attrs != null &&
        typeof vnode3.attrs.onbeforeupdate === 'function'
      ) {
      ...
      if (
        typeof vnode3.tag !== 'string' &&
        typeof vnode3.state.onbeforeupdate === 'function'
      ) {
...
var buildPathname = function (template, params) {
  ...
  var query = {}
  assign(query, params)
  var resolved = path.replace(/:([^\/\.-]+)(\.{3})?/g, function (
    m4,
    key1,
    variadic,
  ) {
...
var compileTemplate = function (template) {
  ...
  return function (data1) {
    ...
    for (var i = 0; i < templateKeys.length; i++) {
      if (
        templateData.params[templateKeys[i]] !== data1.params[templateKeys[i]]
      )
        return false
    }
    ...
    for (var i = 0; i < keys.length; i++) {
      data1.params[keys[i].k] = keys[i].r
        ? values[i + 1]
...
var censor = function (attrs4, extras) {
  ...
  if (extras != null) {
    for (var key7 in attrs4) {
      if (
        hasOwn.call(attrs4, key7) &&
        !magic.test(key7) &&
        extras.indexOf(key7) < 0
      ) {
      ...
      for (; i < compiled.length; i++) {
        if (compiled[i].check(data)) {
          ...
          var update = (lastUpdate = function (comp) {
            ...
            component =
              comp != null &&
              (typeof comp.view === 'function' || typeof comp === 'function')
                ? comp
                : 'div'
